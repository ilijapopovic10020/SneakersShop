...
namespace SneakersShop.Validators
{
    public class RegisterViewModelValidator : AbstractValidator<RegisterViewModel>
    {
        public RegisterViewModelValidator()
        {
            ...
            RuleFor(x => x.Email.Value).NotEmpty().WithMessage("E-adresa je obavezno polje.")
                                 .EmailAddress()
                                 .WithMessage("E-adresa nije u ispravnom formatu.");

            RuleFor(x => x.Username.Value).NotEmpty().WithMessage("Korisničko ime je obavezno polje.")
            ...
            RuleFor(x => x.Password.Value).NotEmpty().WithMessage("Lozinka je obavezno polje.")
                                     .Matches("^(?=.*[A-Za-z])(?=.*\\d)[A-Za-z\\d]{4,}$")
                                     .WithMessage("Lozinka nije u ispravnom formatu.");

            RuleFor(x => x.Phone.Value).NotEmpty().WithMessage("Broj telefona je obavezno polje.")
                         .Matches(@"^06\d{7,8}$")
